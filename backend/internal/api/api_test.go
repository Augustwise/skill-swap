...
func (s *fakeCatalogStore) Ready(context.Context) error {
	return s.readyErr
}

func (s *fakeCatalogStore) Universities(context.Context) ([]university, error) {
	return s.universities, s.universitiesErr
}

func (s *fakeCatalogStore) SkillCategories(context.Context) ([]skillCategory, error) {
	return s.categories, s.categoriesErr
}

func (s *fakeCatalogStore) Skills(_ context.Context, query string, limit int) ([]skill, error) {
	s.lastQuery = query
	s.lastLimit = limit
	return s.skills, s.skillsErr
}

func testHandler(store catalogStore) http.Handler {
	return newHandler(store, "http://localhost:3000", slog.New(slog.NewTextHandler(io.Discard, nil)))
}
...
func TestHealthAndOrigin(t *testing.T) {
	handler := testHandler(&fakeCatalogStore{})
...
func TestOpenAPIContract(t *testing.T) {
	response := request(t, testHandler(&fakeCatalogStore{}), http.MethodGet, "/api/v1/openapi.yaml")
	if response.Code != http.StatusOK {
	...
	for _, fragment := range []string{"openapi: 3.0.3", "/universities:", "/skills:"} {
...
func TestReady(t *testing.T) {
	t.Run("ready", func(t *testing.T) {
		response := request(t, testHandler(&fakeCatalogStore{}), http.MethodGet, "/api/v1/ready")
		if response.Code != http.StatusOK {
			t.Fatalf("status = %d, body = %s", response.Code, response.Body.String())
		}
	...
	t.Run("database unavailable", func(t *testing.T) {
		store := &fakeCatalogStore{readyErr: errDatabaseUnavailable}
...
func TestUniversities(t *testing.T) {
	store := &fakeCatalogStore{universities: []university{{
...
func TestSkillCategories(t *testing.T) {
	store := &fakeCatalogStore{categories: []skillCategory{{
...
func TestSkillsSearchAndLimit(t *testing.T) {
	store := &fakeCatalogStore{skills: []skill{{
...
func TestSkillsRejectsInvalidQuery(t *testing.T) {
	...
	response := request(t, testHandler(&fakeCatalogStore{}), http.MethodGet, target)
...
func TestSkillsRejectsInvalidLimit(t *testing.T) {
	for _, limit := range []string{"0", "101", "-1", "1.5", "many"} {
		t.Run(limit, func(t *testing.T) {
			response := request(t, testHandler(&fakeCatalogStore{}), http.MethodGet,
...
func TestDatabaseUnavailable(t *testing.T) {
	tests := []struct {
		...
		{name: "universities", path: "/api/v1/universities", store: &fakeCatalogStore{universitiesErr: errDatabaseUnavailable}},
		{name: "categories", path: "/api/v1/skill-categories", store: &fakeCatalogStore{categoriesErr: errDatabaseUnavailable}},
		{name: "skills", path: "/api/v1/skills", store: &fakeCatalogStore{skillsErr: errDatabaseUnavailable}},
	}
